    ...
    def _get_column_data(self, f, col: int) -> str:
        if col == 0:
            ...
            if hasattr(f, "response") and f.response:
                ...
                return content_type
            return ""
        elif col == 8:  # Info (e.g. plugin fingerprint info / MCP / lock mark)
            ...
            if meta.get("_mcp"):
                # Sessions created through the MCP server
                text = "MCP " + text if text else "MCP"
            if meta.get("_locked"):
                # Lock mark prefix for locked sessions
